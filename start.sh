...
CONDA_ENV_NAME="aic-final" # <-- 【注意】在这里修改你的 Conda 环境名称
...
# 尝试定位 conda.sh 初始化脚本 (支持 Miniconda 和 Anaconda)
CONDA_PATH=""
...
if [ -n "$CONDA_PATH" ]; then
    ...
    echo "⚠️  Warning: Conda initialization script not found. Trying to use system PATH..."
fi
...
if [ $? -ne 0 ]; then
    ...
    echo "   To create it: conda create -n ${CONDA_ENV_NAME} python=3.12 -y"
