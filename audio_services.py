...
def processor_task():
    """
    [STEP 3 - FINAL & ROBUST FIX]
    This version is hardened to defensively handle data from the queue.
    It checks the type of the item received. If it's a string (the old, incorrect
    format from Alt+G), it manually sets session_id to None. If it's a tuple
    (the correct format), it unpacks it normally. This makes the processor
    resilient to configuration errors and ensures it never crashes from this error again.
    """
    while True:
        try:
            # 从队列中获取任务
            ...
            if isinstance(item, tuple):
                # 如果是元组（来自 Alt+B 或者修复后的 Alt+G），正常解包
                audio_path_str, session_id = item
            elif isinstance(item, str):
                # 如果只是一个字符串（来自未修复的 Alt+G），我们手动处理
                print(f"   -> {Colors.YELLOW}[Compatibility Fix] Received a string item. Assuming short recording.{Colors.ENDC}")
                audio_path_str = item
                session_id = None # 手动将会话 ID 设为 None
            else:
                # 如果是其他未知类型，打印警告并跳过
                print(f"   -> {Colors.RED}[Processor Warning] Received unknown item type from queue: {type(item)}. Skipping.{Colors.ENDC}")
                continue # 跳过这次循环，处理下一个任务
            # --- 【【【 修复结束 】】】 ---
